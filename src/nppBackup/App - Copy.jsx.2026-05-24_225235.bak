...
function AuthModal({onClose,onAuth}) {
  ...
  async function submit() {
    ...
    if (mode==="register") {
      if (!name.trim()||!email.trim()||!pw.trim()) { setErr("All fields are required."); return; }
      if (users[email]) { setErr("An account with this email already exists."); return; }
      const user={id:uid(),name:name.trim(),email:email.trim(),pw,role};
      users[email]=user; await sSet(K.users,users); await sSet(K.session,user); onAuth(user);
    } else {
      if (!email.trim()||!pw.trim()) { setErr("Email and password required."); return; }
      const user=users[email];
      if (!user||user.pw!==pw) { setErr("Incorrect email or password."); return; }
      await sSet(K.session,user); onAuth(user);
    }
  }

  const SocialBtn = ({children,icon}) => (
    <button style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,width:"100%",padding:"11px 16px",border:`1.5px solid ${T.border}`,borderRadius:100,background:T.bg,fontSize:14,fontWeight:500,color:T.text,cursor:"pointer",marginBottom:10,fontFamily:"inherit",transition:"background .15s"}}
      onMouseEnter={e=>e.currentTarget.style.background=T.surface}
      onMouseLeave={e=>e.currentTarget.style.background=T.bg}>
      <span style={{fontSize:17,lineHeight:1}}>{icon}</span>{children}
    </button>
  );

  return (
    <Modal onClose={onClose} title={mode==="login"?"Sign in":"Create account"} subtitle={mode==="login"?"to continue to judge.dog":"Join the judge.dog community"}>
      <SocialBtn icon="🔵">Continue with Google</SocialBtn>
      <SocialBtn icon="📘">Continue with Facebook</SocialBtn>
      <div style={{display:"flex",alignItems:"center",gap:10,margin:"8px 0 16px"}}>
        <div style={{flex:1,height:1,background:T.border}}/><span style={{fontSize:12,color:T.textHint,fontWeight:500}}>or</span><div style={{flex:1,height:1,background:T.border}}/>
      </div>
      {mode==="register"&&(
        <>
          <Field label="Full name" value={name} onChange={e=>setName(e.target.value)} style={{marginBottom:12}}/>
          <div style={{marginBottom:12}}>
            <p style={{margin:"0 0 6px",fontSize:12,fontWeight:500,color:T.textSub}}>I am a…</p>
            <div style={{display:"flex",gap:6}}>
              {[["exhibitor","Exhibitor"],["organizer","Organiser"],["judge","Judge"]].map(([v,l])=>(
                <button key={v} onClick={()=>setRole(v)}
                  style={{flex:1,padding:"8px 4px",borderRadius:100,border:`1.5px solid ${role===v?T.accent:T.border}`,background:role===v?T.accentLight:T.bg,color:role===v?T.accent:T.textSub,fontSize:12,fontWeight:500,cursor:"pointer",fontFamily:"inherit",transition:"all .15s"}}>
                  {l}
                </button>
              ))}
            </div>
      ...
      <Field label="Email" type="email" value={email} onChange={e=>setEmail(e.target.value)} style={{marginBottom:12}}/>
      <Field label="Password" type="password" value={pw} onChange={e=>setPw(e.target.value)} style={{marginBottom:16}}/>
      {err&&<div style={{padding:"10px 14px",background:T.redLight,borderRadius:T.rsm,fontSize:13,color:T.red,marginBottom:14}}>{err}</div>}
      <Btn fullWidth onClick={submit}>{mode==="login"?"Sign in":"Create account"}</Btn>
      <p style={{textAlign:"center",margin:"16px 0 0",fontSize:13,color:T.textSub}}>
        {mode==="login"?"No account? ":"Already have an account? "}
        <span style={{color:T.accent,cursor:"pointer",fontWeight:500}} onClick={()=>{setMode(mode==="login"?"register":"login");setErr("");}}>
...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      ...
      const sb=await sGet(K.bookings,null); const ss=await sGet(K.session,null);
  ...
  const logout=async()=>{setUser(null);await sSet(K.session,null);};
